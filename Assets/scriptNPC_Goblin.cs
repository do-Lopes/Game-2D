...
public class scriptNPC_Goblin : MonoBehaviour
{
    ...
    {       
        if(!isDead){
            ...
            playerHit = Physics2D.Raycast(eyes.transform.position, -transform.right, 1.3f, playerLayer);
            if (playerHit.collider != null)
            {
                Destroy(playerHit.collider.gameObject);
                SceneManager.LoadSceneAsync(2);
            ...
            ccd.radius = 0.07261911f;
            ccd.offset = new Vector2(0, 0);
            anim.SetTrigger("isDead");
            rbd.velocity = new Vector2(0, -0.1f);
            gameObject.layer = 9;
